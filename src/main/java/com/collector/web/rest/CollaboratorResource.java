...
public class CollaboratorResource {
    ...
    public ResponseEntity<CollaboratorDTO> getCollaborator(@PathVariable Long id) {
        ...
        Optional<CollaboratorDTO> collaboratorDTO = collaboratorService.findOne(id);
        return ResponseUtil.wrapOrNotFound(collaboratorDTO);
    }
    ...
    /**
     * SEARCH  /_search/collaborators?query=:query : search for the collaborator corresponding
     * to the query.
     *
     * @param query the query of the collaborator search
     * @return the result of the search
     */
    @GetMapping("/_search/collaborators")
    @Timed
    public List<CollaboratorDTO> searchCollaborators(@RequestParam String query) {
        log.debug("REST request to search Collaborators for query {}", query);
